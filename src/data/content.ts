...
export const services = [
  {
    ...
    description:
      "High-performance websites, AI-assisted development, and modern UI systems — engineered for speed, clarity, and conversion.",
    items: [
      "Frontend Systems & Web Apps",
  ...
  },
  {
    ...
    description:
      "Brand identity, AI-enhanced visuals, and high-impact marketing creatives that make businesses look and feel premium.",
    items: [
...
export const skills = [
  {
    ...
    icon: "Code",
    items: ["React", "TypeScript", "Next.js", "Tailwind CSS", "Node.js", "Vite", "Supabase", "Firebase"],
  },
  {
    category: "Design",
    icon: "Palette",
    items: ["Figma", "Photoshop", "Illustrator", "InDesign", "After Effects"],
  },
  {
    category: "AI & Automation",
    icon: "Sparkles",
    items: ["AI Image Generation", "Prompt Engineering", "Custom Model Training", "AI-Assisted Development"],
  },
];
...
// ---- Unified showpiece data (single source of truth for the hero) ----
...
export const heroShowcase: HeroShowcaseProject[] = (() => {
  const byId = (id: string) => caseStudies.find((c) => c.id === id) as CaseStudy;

  const vwc = byId("valley-wedding-cars");
  const tol = byId("tol-feedback");
  const jai = byId("jugaadvision-ai");

  const techOf = (s: CaseStudy) =>
    Array.from(new Set([...(s.techStack?.development ?? []), ...(s.technologies ?? [])]));
  const imageOf = (s: CaseStudy) => s.screenshots?.[0]?.src || s.image || "";
  ...
  return [
    {
      ...
      tagline: vwc.subtitle,
      metrics: "2,400+",
      metricsLabel: "Inquiries Handled",
      tech: techOf(vwc),
      liveUrl: vwc.liveLink,
      image: imageOf(vwc),
      highlights: (vwc.keyFeatures ?? []).slice(0, 3),
    },
    {
      id: tol.id,
      name: "Tree of Life Feedback",
      category: "QR Feedback & Analytics",
      tagline: tol.subtitle,
      metrics: "4.9/5",
      metricsLabel: "Customer NPS Score",
      tech: techOf(tol),
      liveUrl: tol.liveLink,
      image: imageOf(tol),
      highlights: (tol.keyFeatures ?? []).slice(0, 3),
    },
    {
      id: jai.id,
      name: "JugaadVision AI",
      category: "AI SaaS Platform",
      tagline: jai.subtitle,
      metrics: "5x",
      metricsLabel: "Faster Creation",
      tech: techOf(jai),
      liveUrl: jai.liveLink,
      image: imageOf(jai),
      highlights: (jai.keyFeatures ?? []).slice(0, 3),
    },
  ];
})();
